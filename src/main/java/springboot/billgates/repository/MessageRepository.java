...
public interface MessageRepository extends JpaRepository<Message, Long> {
    ...
    List<Message> findPageNativeFiltered(
        @Param("offset") int offset, 
        @Param("limit") int limit, 
        @Param("channel") String channel
    );

    // 🔹 필터된 조건의 전체 개수를 구하는 쿼리 (페이지네이션 계산용)
    @Query(
        value = "SELECT COUNT(*) FROM MESSAGE WHERE (:channel = 'ALL' OR channel = :channel)", 
        nativeQuery = true
    )
    long countFiltered(@Param("channel") String channel);
